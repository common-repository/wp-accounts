...
$qry[] = "FROM " . $wpdb->prefix . "accounts_invoices INNER JOIN " . $wpdb->prefix . "accounts_clients ON " . $wpdb->prefix . "accounts_invoices.client=" . $wpdb->prefix . "accounts_clients.ID INNER JOIN " . $wpdb->prefix . "accounts_invoice_status ON " . $wpdb->prefix . "accounts_invoices.invoice_status=" . $wpdb->prefix . "accounts_invoice_status.ID INNER JOIN " . $wpdb->prefix . "accounts_payment_methods ON " . $wpdb->prefix . "accounts_invoices.payment_method=" . $wpdb->prefix . "accounts_payment_methods.ID";
$qry[] = "WHERE date_paid >= '" . $accounting_period_start->format('Y-m-d') . "' AND date_paid < '" . $accounting_period_finish->format('Y-m-d')  . "'";
...
$filename = sanitize_title(get_bloginfo('name'))."-invoices-paid-".$accounting_period_start->format('Y-m-d')."-".$accounting_period_finish->format('Y-m-d').".csv";
